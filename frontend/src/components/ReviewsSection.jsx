...
function ReviewsSection({ fieldId }) {
  ...
  async function handleSubmit(e) {
    ...
    if (rating === 0) {
      setFormError("Te rugăm să selectezi un rating.");
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) {
        setFormError(data.mesaj || "Eroare la trimitere.");
        return;
      }

      showToast(
        isEditing ? "Recenzie actualizată! " : "Recenzie salvată! ",
        "success",
      );
      loadReviews();
    } catch {
  ...
  return (
    <div className="reviews-section">
      ...
      {isLoggedIn() ? (
        isEditing ? (
          <div className="review-already-sent">
            Ai trimis deja o recenzie pentru acest teren.
          </div>
        ) : (
        ...
        <div className="review-login-prompt">
          <p>
            {" "}
            <a href="/login">Autentifică-te</a>pentru a lăsa o recenzie.
          </p>
          <p
            style={{
              fontSize: "0.8rem",
          ...
          >
            Poți recenza doar terenuri pe care le-ai rezervat și plătit.
          </p>
        </div>
      )}
        ...
        {loading ? (
          [1, 2].map((i) => (
            <div key={i} className="review-card">
              ...
              <div
                className="skeleton"
                style={{ height: 14, width: "80%", borderRadius: 8 }}
              />
            </div>
          ))
        ) : reviews.length === 0 ? (
