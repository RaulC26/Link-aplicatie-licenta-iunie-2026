  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h2>Toate Rezervările ({filteredBookings.length}{hasActiveFilters ? ` / ${bookings.length}` : ''})</h2>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', minWidth: 110 }}>
          <label style={{ fontSize: '0.72rem', fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 4 }}>
            ID
          </label>
          <input
            type="text"
      ...
      {filteredBookings.length === 0 ? (
        <p style={{ color: '#666' }}>
          {hasActiveFilters ? 'Nicio rezervare nu corespunde filtrelor.' : 'Nu există rezervări.'}
        </p>
      ) : (
              ...
              {filteredBookings.map(booking => (
                <tr key={booking.id}>
                  ...
                  <td>
                    ...
                    <br />
                    <small style={{ color: '#888' }}>{booking.user_email}</small>
                  </td>
