...
function AdminDatePicker({ value, onChange, min, max, placeholder = 'Selectează data' }) {
        ...
        {open && (
          ...
          >
            ...
            <div className="adp-weekdays">
              {WEEKDAYS.map(wd => (
                <span key={wd} className="adp-weekday">{wd}</span>
              ))}
            </div>

            <div className="adp-grid">
              {cells.map((day, i) => {
                if (!day) return <span key={`e-${i}`} className="adp-cell-empty" />
