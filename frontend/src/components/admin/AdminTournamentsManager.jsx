...
function AdminTournamentsManager() {
  ...
  async function loadRegistrations(tournament) {
    ...
    try {
      const res = await fetch(
        `${API_URL}/admin/tournaments/${tournament.id}/registrations`,
        {
          headers: { Authorization: "Bearer " + getToken() },
        },
      );
  ...
  async function handleSaveWinners() {
    ...
    try {
      const res = await fetch(
        `${API_URL}/admin/tournaments/${viewWinners.id}/winners`,
        {
          method: "PUT",
          headers: {
      ...
      const data = await res.json();
      if (!res.ok) {
        setWinnersError(data.mesaj || "Eroare.");
        return;
      }
      setWinnersSuccess("Câștigătorii au fost salvați!");
      loadTournaments();
    } catch {
      setWinnersError("Eroare conexiune.");
    } finally {
  ...
  async function handleDeleteTournament(t) {
    if (
      !window.confirm(
        `Ștergi turneul "${t.name}"? Se șterg și toate înscrierile.`,
      )
    )
      return;
    try {
  ...
  async function handleStatusChange(regId, newStatus) {
    try {
      const res = await fetch(
        `${API_URL}/admin/tournaments/registrations/${regId}/status`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  async function handleDeleteReg(reg) {
    if (!window.confirm(`Elimini echipa "${reg.team_name}" din turneu?`))
      return;
    try {
      const res = await fetch(
        `${API_URL}/admin/tournaments/registrations/${reg.id}`,
        {
          method: "DELETE",
          headers: { Authorization: "Bearer " + getToken() },
        },
  ...
  async function handleSaveReg(regId) {
    ...
    if (!editForm.team_name.trim()) {
      setSaveError("Numele echipei este obligatoriu.");
      return;
    }
    setSaveLoading(true);
    try {
      const res = await fetch(
        `${API_URL}/admin/tournaments/registrations/${regId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  if (viewWinners) {
    const approvedTeams = (viewWinners.approved_team_names || []).filter(
      Boolean,
    );
    ...
    return (
      <div>
        ...
        <div className="profile-card" style={{ maxWidth: 640 }}>
          ...
          >
            Introdu numele echipei câștigătoare pe fiecare loc. Câmpurile sunt
            opționale — poți completa doar locul 1 dacă turneul are doar un
            câștigător desemnat.
          </p>
          ...
          <div className="profile-edit-form">
            ...
            <input
              ...
              onChange={(e) =>
                setWinnersForm((f) => ({ ...f, winner_first: e.target.value }))
              }
            ...
            <input
              ...
              onChange={(e) =>
                setWinnersForm((f) => ({ ...f, winner_second: e.target.value }))
              }
            ...
            <input
              ...
              onChange={(e) =>
                setWinnersForm((f) => ({ ...f, winner_third: e.target.value }))
              }
      ...
      <div>
        ...
        <div className="reg-stats-row">
          ...
          <span style={{ color: "#6b7280", fontSize: "0.85rem" }}>
            Maxim: {viewRegistrations.max_teams} echipe ·{" "}
            {viewRegistrations.team_size} jucători/echipă
          </span>
        </div>
        ...
        {regLoading ? (
          <div className="teams-grid">
            {[1, 2, 3].map((i) => (
              <SkeletonAdminTable key={i} rows={3} cols={4} />
            ))}
          </div>
                ...
                {editingRegId !== reg.id ? (
                  <>
                    <div className="team-card-header">
                      ...
                      <span
                        className={`tournament-status tournament-status-${reg.status === "approved" ? "active" : reg.status === "rejected" ? "cancelled" : "upcoming"}`}
                      >
                        {reg.status === "approved"
                          ? " Aprobată"
                          : reg.status === "rejected"
                    ...
                    >
                      Înscris: {formatDate(reg.created_at)}
                    </p>
                        ...
                        {editForm.players.map((p, j) => (
                          ...
                          >
                            ...
                            <input
                              ...
                              value={p}
                              onChange={(e) =>
                                handlePlayerEdit(j, e.target.value)
                              }
            ...
            <tbody>
              {tournaments.map((t) => (
                <tr key={t.id}>
                  ...
                  <td>
                    {t.registrations_count || 0} / {t.max_teams}
                  </td>
                  <td>{t.team_size} / echipă</td>
                  <td>
                    <span
                      className={`tournament-status tournament-status-${t.status}`}
                    >
                      {t.status === "upcoming"
                        ? "Înregistrări"
                        : t.status === "active"
