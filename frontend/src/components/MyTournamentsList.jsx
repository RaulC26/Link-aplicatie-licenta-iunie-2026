...
function MyTournamentsList() {
  ...
  async function handleCancel(regId, tournamentName) {
    if (!window.confirm(`Anulezi înscrierea la turneul "${tournamentName}"?`))
      return;
    try {
    ...
    return (
      <div className="empty-state">
        ...
        <p style={{ marginBottom: "20px" }}>
          Caută un turneu activ și înscrie-ți echipa!
        </p>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
      {registrations.map((reg) => (
        <div
          key={reg.id}
          className={`my-tournament-card my-tournament-${reg.status}`}
        >
          ...
          <div className="my-tournament-body">
            ...
            <div className="my-tournament-col">
              <p className="my-tournament-section-title">
                Jucătorii echipei ({(reg.players || []).length}/{reg.team_size})
              </p>
          ...
          <div className="my-tournament-footer">
            ...
            {reg.status === "approved" && (
              ...
              >
                Înscrierea ta a fost aprobată de organizatori
              </span>
            )}
          </div>
        </div>
