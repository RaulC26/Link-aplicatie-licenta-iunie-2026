...
function FieldsList() {
  ...
  useEffect(() => {
    async function loadFields() {
      try {
        ...
        setError(
          "Nu s-au putut încărca terenurile. Verifică că backend-ul rulează.",
        );
      } finally {
        setLoading(false);
      }
  ...
  if (maxPrice !== "") {
    filtered = filtered.filter(
      (f) => parseFloat(f.price_per_hour) <= parseFloat(maxPrice),
    );
  }
  ...
  const maxFieldPrice = Math.max(
    ...fields.map((f) => parseFloat(f.price_per_hour)),
    0,
  );
