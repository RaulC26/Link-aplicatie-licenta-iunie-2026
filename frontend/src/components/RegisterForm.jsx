...
function RegisterForm() {
  ...
  async function handleSubmit(e) {
    ...
    if (!name || !email || !password) {
      setError("Numele, emailul și parola sunt obligatorii.");
      return;
    }
    ...
    if (!numeRegex.test(name.trim())) {
      setError("Numele trebuie să conțină doar litere.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        setError(data.mesaj || "Eroare la înregistrare.");
        return;
      }
  ...
  return (
    <form onSubmit={handleSubmit}>
        ...
        {password && (
          <ul className="password-rules">
            <li className={pwRules.length ? "rule-ok" : "rule-bad"}>
              {pwRules.length ? <Check size={13} /> : <XIcon size={13} />} Minim
              8 caractere
            </li>
            <li className={pwRules.uppercase ? "rule-ok" : "rule-bad"}>
              {pwRules.uppercase ? <Check size={13} /> : <XIcon size={13} />}{" "}
              Cel puțin o literă mare (A-Z)
            </li>
            <li className={pwRules.digit ? "rule-ok" : "rule-bad"}>
              {pwRules.digit ? <Check size={13} /> : <XIcon size={13} />} Cel
              puțin o cifră (0-9)
            </li>
            <li className={pwRules.special ? "rule-ok" : "rule-bad"}>
              {pwRules.special ? <Check size={13} /> : <XIcon size={13} />} Cel
              puțin un simbol (!@#$ etc.)
            </li>
          </ul>
        )}
      ...
      <div className="auth-input-group">
        <label>
          Telefon <span className="auth-optional">(opțional)</span>
        </label>
        <div className="auth-input-wrapper">
        ...
        {phone && phone.length > 0 && phone.length < 10 && (
          ...
          >
            Mai trebuie {10 - phone.length}{" "}
            {10 - phone.length === 1 ? "cifră" : "cifre"}
          </small>
        )}
      </div>
