...
const statusLabel = { upcoming: 'Înscrieri deschise', active: 'În desfășurare', completed: 'Finalizat', cancelled: 'Anulat' }
const statusClass = { upcoming: 'tournament-status-upcoming', active: 'tournament-status-active', completed: 'tournament-status-completed', cancelled: 'tournament-status-cancelled' }
const statusDot = { upcoming: '🟢', active: '🔵', completed: '⚫', cancelled: '🔴' }
...
function TournamentsPage() {
 ...
 if (!raw) return '—'
 return new Date(String(raw).substring(0, 10) + 'T12:00:00')
 ...
 if (!raw) return null
 const diff = Math.ceil((new Date(String(raw).substring(0, 10) + 'T23:59:59') - new Date()) / 86400000)
 return diff
 }
 ...
 {[1, 2, 3, 4].map(i => <SkeletonTournamentCard key={i} />)}
 </div>
 </div>
 )
 ...
 <p>Înscrie-ți echipa la competițiile organizate și luptă pentru victorie! Fiecare meci contează.</p>
 </motion.div>
 ...
 {f.count !== undefined && (
 <span style={{ marginLeft: 4, background: 'rgba(255,255,255,0.2)', borderRadius: 99, padding: '1px 7px', fontSize: '0.75rem' }}>
 {f.count}
 </span>
 )}
 </button>
 ...
 <p>Nu există turnee în această categorie. Revino mai târziu pentru noi competiții.</p>
 </motion.div>
 ) : (
 ...
 const spotsLeft = Math.max(0, t.max_teams - (t.registrations_count || 0))
 const fillPct = Math.min(100, Math.round(((t.registrations_count || 0) / t.max_teams) * 100))
 ...
 <span className={`tournament-status ${statusClass[t.status] || ''}`}>
 ...
 <div className="tournament-meta-item">
 <span className="tournament-meta-icon">👥</span>
 <span><strong>{t.team_size}</strong>jucători/echipă</span>
 </div>
 {t.prize_info && (
 <div className="tournament-meta-item">
 <span className="tournament-meta-icon"></span>
 ...
 <span style={{ color: spotsLeft > 0 ? '#16a34a' : '#dc2626', fontWeight: 700 }}>
 {spotsLeft > 0 ? `${spotsLeft} locuri libere` : '🔴 Complet'}
 </span>
 </div>
 <div className="tournament-spots-track">
 <div className="tournament-spots-fill" style={{ width: `${fillPct}%` }} />
 </div>
 </div>

 <div className="tournament-card-footer">
 <Link to={`/tournaments/${t.id}`} className="btn-primary">
 {t.status === 'upcoming' ? ' Înscrie echipa' : '👀 Vezi detalii'}
 </Link>
 </div>
 </div>
 </motion.div>
