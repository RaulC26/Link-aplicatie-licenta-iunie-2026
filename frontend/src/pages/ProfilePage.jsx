...
import { getToken, getUserFromToken, removeToken, saveToken } from '../utils/auth'
...
function ProfilePage() {
 ...
 if (tokenUser) setUser({ name: tokenUser.name || tokenUser.email, email: tokenUser.email })
 } finally {
 ...
 if (!editName.trim()) { setSaveError('Numele nu poate fi gol.'); return }
 ...
 setPassError('Completează toate câmpurile.'); return
 }
 if (newPassword !== confirmPassword) {
 setPassError('Noua parolă și confirmarea nu coincid.'); return
 }
 if (!allPwRulesValid) {
 setPassError('Noua parolă nu îndeplinește toate cerințele de securitate.'); return
 }
 ...
 const data = await res.json()
 if (!res.ok) { setPassError(data.mesaj || 'Eroare.'); return }

 setPassSuccess('Parola a fost schimbată cu succes!')
 setCurrentPassword(''); setNewPassword(''); setConfirmPassword('')
 } catch { setPassError('Eroare conexiune.') }
 ...
 {user?.phone && <span className="profile-hero-meta-item"> {user.phone}</span>}
 </div>
 <p className="profile-joined">Membru din {formatDate(user?.created_at)}</p>
 </div>
 {user?.role === 'admin' && (
 ...
 onClick={() => { setActiveTab('info'); setEditMode(false); setSaveSuccess(''); setSaveError('') }}
 > Date personale
 </button>
 <button
 className={activeTab === 'password' ? 'profile-tab active' : 'profile-tab'}
 onClick={() => { setActiveTab('password'); setPassSuccess(''); setPassError('') }}
 > Schimbă parola
 </button>
 </div>
 ...
 setEditMode(true); setSaveSuccess(''); setSaveError('')
 }}>Editează profilul
 </button>
 </div>
 ...
 <div style={{ marginTop: '24px', paddingTop: '20px', borderTop: '1px solid var(--bg)', textAlign: 'right' }}>
 <button
 ...
 />
 <small style={{ color: '#9ca3af', marginTop: '-12px', marginBottom: '12px', display: 'block' }}>Emailul nu poate fi modificat.
 </small>
 ...
 {pwRules.uppercase ? <Check size={13} /> : <XIcon size={13} />} Cel puțin o literă mare
 </li>
 <li className={pwRules.digit ? 'rule-ok' : 'rule-bad'}>
 {pwRules.digit ? <Check size={13} /> : <XIcon size={13} />} Cel puțin o cifră
 </li>
 <li className={pwRules.special ? 'rule-ok' : 'rule-bad'}>
 {pwRules.special ? <Check size={13} /> : <XIcon size={13} />} Cel puțin un simbol
 </li>
 </ul>
 )}
 ...
 <button type="submit" className="btn-primary" disabled={passLoading} style={{ marginTop: '8px' }}>
