...
router.post("/", verifyToken, async (req, res) => {
  ...
  if (!field_id || !booking_date || !start_time) {
    return res
      .status(400)
      .json({ mesaj: "field_id, booking_date și start_time sunt obligatorii" });
  }
  ...
  if (!timeRegex.test(start_time)) {
    return res
      .status(400)
      .json({
        mesaj: "start_time trebuie să fie în formatul HH:MM (ex: 10:00)",
      });
  }
  ...
  if (startM !== 0) {
    return res
      .status(400)
      .json({
        mesaj: "Rezervările se fac pe ore fixe (ex: 10:00, 11:00, etc.)",
      });
  }
  ...
  if (startH < 8 || startH > 21) {
    return res
      .status(400)
      .json({ mesaj: "Rezervările se pot face între 08:00 și 21:00" });
  }
  ...
  try {
    ...
    if (bookingDate < today) {
      return res
        .status(400)
        .json({ mesaj: "Nu poți face rezervări pentru date trecute" });
    }
    ...
    if (booking_date === todayStr) {
      ...
      if (start_time <= nowHHMM) {
        return res
          ...
          .json({
            mesaj:
              "Nu poți rezerva un slot orar care a trecut deja. Alege o oră viitoare.",
          });
      }
    }
    ...
    if (conflict.length > 0) {
      return res
        .status(400)
        .json({ mesaj: "Slotul este deja rezervat. Alege alt interval orar." });
    }
    ...
    try {
      ...
      const [fieldRows] = await db.query(
        "SELECT id, name, location FROM fields WHERE id = ?",
        [field_id],
      );
...
router.delete("/:id", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (booking.user_id !== userId) {
      return res
        .status(403)
        .json({ mesaj: "Nu poți anula rezervări care nu îți aparțin" });
    }

    if (booking.status === "cancelled" || booking.status === "completed") {
      return res
        .status(400)
        .json({ mesaj: "Această rezervare nu poate fi anulată" });
    }
    ...
    if (bookingDateStr === todayStr) {
      ...
      if (bookingStart <= nowHHMM) {
        return res
          ...
          .json({
            mesaj:
              "Nu poți anula o rezervare care a început deja sau a trecut.",
          });
      }
    }
    ...
    try {
      ...
      const [fieldRows] = await db.query(
        "SELECT name, location FROM fields WHERE id = ?",
        [booking.field_id],
      );
      if (userRows.length > 0 && fieldRows.length > 0) {
...
router.post("/multi", verifyToken, async (req, res) => {
  ...
  if (
    ...
    !booking_date ||
    !start_times ||
    !Array.isArray(start_times) ||
    start_times.length === 0
  ) {
    return res
      .status(400)
      .json({
        mesaj: "field_id, booking_date și start_times (array) sunt obligatorii",
      });
  }
  ...
  if (start_times.length > 4) {
    return res
      .status(400)
      .json({ mesaj: "Poți rezerva maxim 4 sloturi consecutive (4 ore)" });
  }
  ...
  for (let i = 0; i < sortedTimes.length - 1; i++) {
    ...
    if (nextH - currentH !== 1) {
      return res
        .status(400)
        .json({
          mesaj: "Sloturile trebuie să fie consecutive (ore consecutive)",
        });
    }
  }
  ...
  try {
    const [fields] = await db.query(
      "SELECT id, price_per_hour, name, location FROM fields WHERE id = ?",
      [field_id],
    );
    if (fields.length === 0)
      return res.status(404).json({ mesaj: "Terenul nu a fost găsit" });
    ...
    if (booking_date === todayStr) {
      ...
      if (sortedTimes[0] <= nowHHMM) {
        return res
          ...
          .json({
            mesaj:
              "Nu poți rezerva sloturi care au trecut deja. Alege ore viitoare.",
          });
      }
    }
    ...
    const lastH =
      parseInt(sortedTimes[sortedTimes.length - 1].split(":")[0]) + 1;
    ...
    if (conflict.length > 0) {
      return res
        .status(400)
        .json({
          mesaj: `Unul sau mai multe sloturi din intervalul ${firstSlot}–${lastSlot} sunt deja rezervate. Alege alte ore.`,
        });
    }
