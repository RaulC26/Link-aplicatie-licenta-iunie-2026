...
router.post('/:id/register', verifyToken, async (req, res) => {
  ...
  if (!players || !Array.isArray(players) || players.length === 0) {
    return res.status(400).json({ mesaj: 'Trebuie să adaugi cel puțin un jucător' });
  }
  ...
  try {
    ...
    if (tournaments.length === 0) {
      return res.status(404).json({ mesaj: 'Turneul nu există sau nu mai acceptă înscrieri' });
    }
    ...
    if (countRows[0].cnt >= tournament.max_teams) {
      return res.status(400).json({ mesaj: 'Turneul este complet. Nu mai sunt locuri disponibile.' });
    }
    ...
    if (existing.length > 0) {
      return res.status(400).json({ mesaj: 'Ești deja înscris la acest turneu' });
    }
    ...
    if (invalidPlayers.length > 0) {
      return res.status(400).json({ mesaj: 'Toate numele jucătorilor sunt obligatorii' });
    }

    const [result] = await db.query(
      `INSERT INTO tournament_registrations (tournament_id, user_id, team_name, captain_name, captain_phone, notes)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [tournamentId, userId, team_name.trim(), captain_name ? captain_name.trim() : null, captain_phone || null, notes || null]
    );
...
router.delete('/registrations/:id', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (rows[0].user_id !== userId) {
      return res.status(403).json({ mesaj: 'Nu poți anula înscrierea altui utilizator' });
    }

    if (rows[0].status === 'approved') {
      return res.status(400).json({ mesaj: 'Înscrierea aprobată nu poate fi anulată. Contactează organizatorii.' });
    }
