...
router.post('/field/:fieldId', verifyToken, async (req, res) => {
  ...
  if (!rating || isNaN(ratingNum) || ratingNum < 1 || ratingNum > 5) {
    return res.status(400).json({ mesaj: 'Rating-ul trebuie să fie un număr întreg între 1 și 5' });
  }

  if (comment && comment.length > 1000) {
    return res.status(400).json({ mesaj: 'Comentariul este prea lung (max 1000 caractere)' });
  }
  ...
  try {
    ...
    if (bookings.length === 0) {
      return res.status(403).json({ mesaj: 'Poți recenza doar terenuri pe care le-ai rezervat și plătit.' });
    }
...
router.delete('/:id', verifyToken, async (req, res) => {
  ...
  const userId = req.user.userId;
  try {
    const [rows] = await db.query('SELECT user_id FROM field_reviews WHERE id = ?', [id]);
    if (rows.length === 0) return res.status(404).json({ mesaj: 'Recenzia nu există' });

    const [user] = await db.query('SELECT role FROM users WHERE id = ?', [userId]);
    if (rows[0].user_id !== userId && user[0]?.role !== 'admin') {
      return res.status(403).json({ mesaj: 'Nu ai permisiunea să ștergi această recenzie' });
    }
