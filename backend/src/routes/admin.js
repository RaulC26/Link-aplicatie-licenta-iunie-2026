...
router.post("/fields", async (req, res) => {
  ...
  if (!name || !location || !price_per_hour) {
    return res
      .status(400)
      .json({ mesaj: "Numele, locația și prețul sunt obligatorii" });
  }

  if (name.trim().length < 3) {
    return res
      .status(400)
      .json({ mesaj: "Numele trebuie să aibă minim 3 caractere" });
  }
  ...
  if (isNaN(price) || price <= 0) {
    return res
      .status(400)
      .json({ mesaj: "Prețul trebuie să fie un număr pozitiv" });
  }
  ...
  if (image_url && image_url.trim() !== "") {
    if (!image_url.startsWith("http://") && !image_url.startsWith("https://")) {
      return res
        .status(400)
        .json({
          mesaj: "URL-ul imaginii trebuie să înceapă cu http:// sau https://",
        });
    }
  }
  ...
  if (lat !== null && (isNaN(lat) || lat < -90 || lat > 90)) {
    return res
      .status(400)
      .json({ mesaj: "Latitudinea trebuie să fie între -90 și 90" });
  }
  if (lng !== null && (isNaN(lng) || lng < -180 || lng > 180)) {
    return res
      .status(400)
      .json({ mesaj: "Longitudinea trebuie să fie între -180 și 180" });
  }
  ...
  try {
    ...
    const [newField] = await db.query("SELECT * FROM fields WHERE id = ?", [
      result.insertId,
    ]);
...
router.put("/fields/:id", async (req, res) => {
  ...
  try {
    const [existing] = await db.query("SELECT id FROM fields WHERE id = ?", [
      fieldId,
    ]);
  ...
  try {
    ...
    const [updated] = await db.query("SELECT * FROM fields WHERE id = ?", [
      fieldId,
    ]);
    res.json(updated[0]);
  } catch (error) {
...
router.put("/users/:id", async (req, res) => {
  ...
  if (parseInt(userId) === adminId && role && role !== "admin") {
    return res
      .status(400)
      .json({
        mesaj: "Nu îți poți schimba propriul rol de admin (te-ai bloca afară).",
      });
  }
  ...
  if (!name || name.trim().length < 2) {
    return res
      .status(400)
      .json({ mesaj: "Numele este obligatoriu (minim 2 caractere)." });
  }

  if (role && !["user", "admin"].includes(role)) {
    return res
      .status(400)
      .json({ mesaj: 'Rolul trebuie să fie "user" sau "admin".' });
  }
...
router.post("/tournaments", async (req, res) => {
  ...
  if (
    ...
    return res
      ...
      .json({
        mesaj:
          "Câmpurile obligatorii: name, start_date, end_date, registration_deadline, max_teams, team_size",
      });
  }
  ...
  if (start_date < todayStrCreate) {
    return res
      .status(400)
      .json({ mesaj: "Data de start nu poate fi în trecut" });
  }
  if (registration_deadline < todayStrCreate) {
    return res
      .status(400)
      .json({ mesaj: "Termenul limită de înscriere nu poate fi în trecut" });
  }
  if (end_date < start_date) {
    return res
      .status(400)
      .json({ mesaj: "Data de final trebuie să fie după data de start" });
  }
  if (registration_deadline > start_date) {
    return res
      .status(400)
      .json({
        mesaj: "Termenul limită trebuie să fie înainte de data de start",
      });
  }
...
router.put("/tournaments/:id", async (req, res) => {
  ...
  try {
    const [check] = await db.query(
      "SELECT start_date, end_date, registration_deadline, status FROM tournaments WHERE id = ?",
      [tournamentId],
    );
    if (check.length === 0)
      return res.status(404).json({ mesaj: "Turneul nu a fost găsit" });

    const existingStart = check[0].start_date
      ? String(check[0].start_date).substring(0, 10)
      : null;
    const existingEnd = check[0].end_date
      ? String(check[0].end_date).substring(0, 10)
      : null;
    const existingDeadline = check[0].registration_deadline
      ? String(check[0].registration_deadline).substring(0, 10)
      : null;
    const todayStr = new Date().toISOString().split("T")[0];
    if (
      existingEnd &&
      existingEnd < todayStr &&
    ...
    if (start_date < todayStr && start_date !== existingStart) {
      return res
        .status(400)
        .json({
          mesaj: "Data de start nu poate fi schimbată la o dată în trecut",
        });
    }
    if (
      registration_deadline < todayStr &&
      registration_deadline !== existingDeadline
    ) {
      return res
        .status(400)
        .json({
          mesaj: "Termenul limită nu poate fi schimbat la o dată în trecut",
        });
    }
    if (end_date < start_date) {
      return res
...
router.delete("/tournaments/:id", async (req, res) => {
  const tournamentId = req.params.id;
  try {
    const [check] = await db.query("SELECT id FROM tournaments WHERE id = ?", [
      tournamentId,
    ]);
    if (check.length === 0)
      return res.status(404).json({ mesaj: "Turneul nu a fost găsit" });
...
router.put("/tournaments/registrations/:id/status", async (req, res) => {
  ...
  if (!["approved", "rejected", "pending"].includes(status)) {
    return res
      .status(400)
      .json({ mesaj: "Status invalid. Folosiți: approved, rejected, pending" });
  }
  ...
  try {
    ...
    if (
      ...
      reg.tournament_status === "cancelled"
    ) {
      return res.status(400).json({
    ...
    res.json({
      mesaj: `Înregistrare ${status === "approved" ? "aprobată" : status === "rejected" ? "respinsă" : "resetată"}!`,
    });
  } catch (e) {
...
router.put("/tournaments/registrations/:id", async (req, res) => {
  const regId = req.params.id;
  const { team_name, captain_name, captain_phone, notes, players, status } =
    req.body;
  ...
  try {
    const [check] = await db.query(
      "SELECT id FROM tournament_registrations WHERE id = ?",
      [regId],
    );
    if (check.length === 0)
      return res.status(404).json({ mesaj: "Înscrierea nu a fost găsită" });
    ...
    if (players && Array.isArray(players) && players.length > 0) {
      await db.query(
        "DELETE FROM tournament_players WHERE registration_id = ?",
        [regId],
      );
...
router.delete("/tournaments/registrations/:id", async (req, res) => {
  ...
  try {
    const [check] = await db.query(
      "SELECT id, team_name FROM tournament_registrations WHERE id = ?",
      [regId],
    );
    if (check.length === 0)
      return res.status(404).json({ mesaj: "Înscrierea nu a fost găsită" });
    ...
    res.json({
      mesaj: `Echipa "${check[0].team_name}" a fost eliminată din turneu!`,
    });
  } catch (e) {
...
router.put("/tournaments/:id/winners", async (req, res) => {
  ...
  try {
    ...
    const first =
      winner_first && winner_first.trim() ? winner_first.trim() : null;
    const second =
      winner_second && winner_second.trim() ? winner_second.trim() : null;
    const third =
      winner_third && winner_third.trim() ? winner_third.trim() : null;
