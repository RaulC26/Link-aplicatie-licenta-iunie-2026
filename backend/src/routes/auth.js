...
router.post("/register", async (req, res) => {
  ...
  if (!name || !email || !password) {
    return res
      .status(400)
      .json({ mesaj: "Numele, emailul și parola sunt obligatorii" });
  }
  ...
  if (!trimmedName) {
    return res
      .status(400)
      .json({ mesaj: "Numele nu poate fi doar spații goale" });
  }

  if (trimmedName.length < 2 || trimmedName.length > 100) {
    return res
      .status(400)
      .json({ mesaj: "Numele trebuie să aibă între 2 și 100 de caractere" });
  }
  if (trimmedEmail.length > 150) {
    return res
      .status(400)
      .json({ mesaj: "Adresa de email este prea lungă (max 150 caractere)" });
  }
  ...
  if (password.length < 8) {
    return res
      .status(400)
      .json({ mesaj: "Parola trebuie să aibă minim 8 caractere" });
  }
  if (password.length > 72) {
    return res
      .status(400)
      .json({ mesaj: "Parola este prea lungă (max 72 caractere)" });
  }
  if (!/[A-Z]/.test(password)) {
    return res
      .status(400)
      .json({ mesaj: "Parola trebuie să conțină cel puțin o literă mare" });
  }
  if (!/[0-9]/.test(password)) {
    return res
      .status(400)
      .json({ mesaj: "Parola trebuie să conțină cel puțin o cifră" });
  }
  if (!/[^A-Za-z0-9]/.test(password)) {
    return res
      ...
      .json({
        mesaj:
          "Parola trebuie să conțină cel puțin un simbol special (!@#$ etc.)",
      });
  }
  ...
  if (trimmedPhone && !/^07[0-9]{8}$/.test(trimmedPhone)) {
    return res
      ...
      .json({
        mesaj:
          "Numărul de telefon trebuie să aibă 10 cifre și să înceapă cu 07 (ex: 0712345678)",
      });
  }
  ...
  try {
    ...
    if (existingUsers.length > 0) {
      return res
        .status(400)
        .json({ mesaj: "Există deja un cont cu acest email" });
    }
...
router.post("/login", async (req, res) => {
  ...
  if (!email || !password) {
    return res
      .status(400)
      .json({ mesaj: "Emailul și parola sunt obligatorii" });
  }
...
router.put("/profile", verifyToken, async (req, res) => {
  ...
  if (name.trim().length < 2) {
    return res
      .status(400)
      .json({ mesaj: "Numele trebuie să aibă minim 2 caractere" });
  }

  if (name.trim().length > 100) {
    return res
      .status(400)
      .json({ mesaj: "Numele este prea lung (max 100 caractere)" });
  }
...
router.put("/change-password", verifyToken, async (req, res) => {
  ...
  if (newPassword !== confirmPassword) {
    return res
      .status(400)
      .json({ mesaj: "Noua parolă și confirmarea nu coincid" });
  }

  if (newPassword.length < 8) {
    return res
      .status(400)
      .json({ mesaj: "Noua parolă trebuie să aibă minim 8 caractere" });
  }
  ...
  try {
    ...
    if (sameAsOld) {
      return res
        .status(400)
        .json({ mesaj: "Noua parolă trebuie să fie diferită de cea actuală" });
    }
...
router.delete("/me", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const [me] = await db.query("SELECT role FROM users WHERE id = ?", [
      userId,
    ]);
    if (me.length === 0)
      return res.status(404).json({ mesaj: "Cont inexistent" });
    ...
    if (me[0].role === "admin") {
      ...
      if (admins[0].cnt <= 1) {
        return res
          .status(400)
          .json({
            mesaj: "Ești singurul administrator. Nu poți șterge contul.",
          });
      }
    }
    ...
    await db.query("DELETE FROM tournament_registrations WHERE user_id = ?", [
      userId,
    ]);
    ...
    res
      .status(500)
      .json({ mesaj: "Eroare la ștergerea contului: " + error.message });
  }
});
...
router.post("/reset-password", async (req, res) => {
  ...
  if (!email || !token || !newPassword) {
    return res
      .status(400)
      .json({ mesaj: "Email, token și parola sunt obligatorii" });
  }
  ...
  if (!/[^A-Za-z0-9]/.test(newPassword)) {
    return res
      .status(400)
      .json({ mesaj: "Parola trebuie să conțină cel puțin un simbol special" });
  }
  ...
  try {
    ...
    res.json({
      mesaj: "Parola a fost resetată cu succes! Te poți autentifica.",
    });
  } catch (error) {
