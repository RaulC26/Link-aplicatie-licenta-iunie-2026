...
router.get('/', async (req, res) => {
  try {
    ...
    res.status(500).json({ mesaj: 'Eroare la citirea terenurilor din baza de date' });
  }
});
...
router.get('/:id', async (req, res) => {
  ...
  try {
    const [rows] = await db.query('SELECT * FROM fields WHERE id = ?', [fieldId]);
...
router.get('/:id/slots', async (req, res) => {
  ...
  if (!date) {
    return res.status(400).json({ mesaj: 'Parametrul ?date=YYYY-MM-DD este obligatoriu' });
  }

  const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
  if (!dateRegex.test(date)) {
    return res.status(400).json({ mesaj: 'Formatul datei trebuie să fie YYYY-MM-DD' });
  }
