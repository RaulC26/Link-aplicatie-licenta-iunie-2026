...
router.post('/create-checkout-session', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (booking.user_id !== userId) {
      return res.status(403).json({ mesaj: 'Nu poți plăti rezervări care nu îți aparțin' });
    }
    ...
    if (existingPayments.length > 0) {
      return res.status(400).json({ mesaj: 'Această rezervare a fost deja plătită' });
    }

    const bookingDateStr = String(booking.booking_date).substring(0, 10);
    const todayStr = new Date().toISOString().split('T')[0];
    if (bookingDateStr < todayStr) {
      return res.status(400).json({ mesaj: 'Nu poți plăti o rezervare trecută.' });
    }
    if (bookingDateStr === todayStr) {
      ...
      if (bookingStart <= nowHHMM) {
        return res.status(400).json({ mesaj: 'Nu poți plăti o rezervare care a început deja sau a trecut.' });
      }
    }
    ...
    res.status(500).json({ mesaj: 'Eroare la inițierea plății. Încearcă din nou.' });
  }
});
